function varargout = spmk_tiv(varargin)
...
% Check whether images come from lga or lpa
...
for i = 1:numel(Vles)
    ...
    volfactor = abs(det(Vles_tmp.mat(1:3,1:3))) /  1000;
    % Consider only those voxels that are larger than 15 microliters
    if any(les(:) > thr)
        bw = ps_bwlabeln(1. * (les > thr));
        les_size = zeros(max(bw(:)), 1);
        for j = 1:max(bw(:))
            les_size(j) = sum(bw(:) == j) * volfactor;
        end
        les_size = les_size(les_size > 0.005);
    else
        les_size = [];
    end    
